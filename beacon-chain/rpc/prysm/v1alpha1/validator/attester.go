...
// GetAttestationData请求beacon node生成一个attestation data对象，之后validator作为一个attester
// 就会进行签名
func (vs *Server) GetAttestationData(ctx context.Context, req *ethpb.AttestationDataRequest) (*ethpb.AttestationData, error) {
	...
	// 一个乐观的validator必须不能参与到attestation
	...
	// 万一我们接受到一个attestation request，在一个更新的state/block已经被处理之后
	...
	res = &ethpb.AttestationData{
		...
		// 将当前的justified checkpoint作为source
...
// ProposeAttestation是一个由attester调用的函数，来对一个block进行vote，通过一个attestation对象
// 在Ethereum Serenity specification中被定义
func (vs *Server) ProposeAttestation(ctx context.Context, att *ethpb.Attestation) (*ethpb.AttestResponse, error) {
	...
	// 广播unaggregated attestation到一个feed，来通知beacon node中的其他services，对于接收到一个unaggregated
	// attestation
	...
	// 决定广播attestation到哪个subnet
	wantedEpoch := slots.ToEpoch(att.Data.Slot)
	vals, err := vs.HeadFetcher.HeadValidatorsIndices(ctx, wantedEpoch)
	if err != nil {
		return nil, err
	}
	// 基于committee和slot计算subnet
...
// SubscribeCommitteeSubnets订阅committee ID subset，给定subscribe request

...
type BeaconChainConfig struct {
	...
	DepositContractTreeDepth uint64      `yaml:"DEPOSIT_CONTRACT_TREE_DEPTH"` // DepositContractTreeDepth depth of the Merkle trie of deposits in the validator deposit contract on the PoW chain. // DepositContractTreeDepth是deposit的Merkle trie但是深度，在PoW chain中
	...
	MinGenesisActiveValidatorCount uint64 `yaml:"MIN_GENESIS_ACTIVE_VALIDATOR_COUNT" spec:"true"` // MinGenesisActiveValidatorCount defines how many validator deposits needed to kick off beacon chain. // 决定了需要多少个validator deposits来启动beacon chain
	...
	// HysteresisDownwardMultiplier定义了hysteresis downward multiplier，对于effective balance的计算
	...
	MaxEffectiveBalance       uint64 `yaml:"MAX_EFFECTIVE_BALANCE" spec:"true"`       // MaxEffectiveBalance is the maximal amount of Gwei that is effective for staking. // MaxEffectiveBalance是对于staking有效的最大数目的Gwei
	...
	Eth1FollowDistance               uint64      `yaml:"ETH1_FOLLOW_DISTANCE" spec:"true"`                // Eth1FollowDistance is the number of eth1.0 blocks to wait before considering a new deposit for voting. This only applies after the chain as been started. // Eth1FollowDistance是eth1.0 blocks的数目在考虑一个新的deposit可以voting之前，这只能在chain启动之后应用
	// SafeSlotsToUpdateJustified是更新justified checkpoint所需的最少的slots
	...
	// DepositContractAddress是部署deposit contract的地址
